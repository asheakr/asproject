...
public:
    LandingController() 
        : nh_("~"), has_current_state_(false), current_landing_state_(LandingState::IDLE) {
    ...
    void currentStateCallback(const nav_msgs::Odometry::ConstPtr& msg) {
        ...
        if (current_landing_state_ == LandingState::IDLE) {
    ...
    void takeoffCommandCallback(const std_msgs::Empty::ConstPtr&) {
        if (current_landing_state_ == LandingState::IDLE || current_landing_state_ == LandingState::LANDED) {
            ROS_INFO("Takeoff command received.");
            current_landing_state_ = LandingState::TAKEOFF;
            state_entry_time_ = ros::Time::now();
            ...
            if (!has_current_state_) {
                ROS_WARN("No odometry received yet — using default takeoff position (0,0).");
                target_x_ = 0.0;
                target_y_ = 0.0;
                target_z_ = takeoff_height_;
            }
        } else {
            ROS_WARN("Takeoff command ignored. Current state: %d", static_cast<int>(current_landing_state_));
        }
    }

    void landCommandCallback(const std_msgs::Empty::ConstPtr&) {
        if (current_landing_state_ == LandingState::HOVER) {
            ROS_INFO("Land command received.");
            current_landing_state_ = LandingState::LANDING;
            state_entry_time_ = ros::Time::now();
    ...
    void executeTakeoff() {
        if (has_current_state_ && current_state_.pose.pose.position.z >= takeoff_height_ - 0.1) {
            current_landing_state_ = LandingState::HOVER;
            ROS_INFO("Takeoff complete. Transitioning to HOVER.");
            return;
        }

        publishDesiredState(target_x_, target_y_, takeoff_height_, takeoff_velocity_);
    }

    void executeHover() {
        publishDesiredState(target_x_, target_y_, hover_height_, 0.0);
    }

    void executeLanding() {
        if (has_current_state_ && current_state_.pose.pose.position.z <= ground_threshold_) {
            current_landing_state_ = LandingState::LANDED;
            ROS_INFO("Landing complete. Transitioning to LANDED.");
            return;
        }

        publishDesiredState(target_x_, target_y_, 0.0, -landing_velocity_);
    }

    void publishDesiredState(double x, double y, double z, double vz) {
        ...
        point.transforms[0].rotation = current_state_.pose.pose.orientation;

        point.velocities[0].linear.z = vz;

        desired_state_pub_.publish(point);
    }
